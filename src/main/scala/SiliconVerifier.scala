...
class SiliconVerifier[O,Err] extends SilverImplementation[O,Err] {
  ...
  override def createVerifier(tool_home: Path,props: Properties):viper.silver.verifier.Verifier = {
    ...
    val z3_exe=if(OS.startsWith("Windows")){
      tool_home.resolve("z3").resolve("4.4.1").resolve("Windows NT").resolve("intel").resolve("bin").resolve("z3.exe").toString()
    } else if(OS.startsWith("Mac")){
      tool_home.resolve("z3").resolve("4.4.1").resolve("Darwin").resolve("x86_64").resolve("bin").resolve("z3").toString()
    } else {
      tool_home.resolve("z3").resolve("4.4.1").resolve("Linux").resolve("x86_64").resolve("bin").resolve("z3").toString()
    }
